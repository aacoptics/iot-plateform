      ...
      setTimeout(() => {
        this.$nextTick(()=>{
          this.areaCode.forEach((item) => {
            this.drawOEEPieChart(`oee${item}`, this.lastDayTotalTime[item])
          });
        });
      }, 1000);
    },
    drawOEEPieChart(name, OEEValue) {
      let oeePieChart = this.$echarts.init(document.getElementById(name));
      var option = {
        tooltip: {
          trigger: 'item'
        },
        ...
        series: [
          {
            ...
            data: [
              { value: OEEValue, name: name.slice(3)+' OEE(%)'},
              { value: 100-OEEValue, name: ''}
            ],
            itemStyle: {
              normal: {
                label: {
      ...
      oeePieChart.setOption(option);
    },
    getAbnormalToolLifeRatio() {
      getAbnormalToolLifeRatio().then((response) => {
        const responseData = response.data
        console.log(responseData)
        if (responseData.code === '000000') {
          this.abnormalMatCode = responseData.data.matCode;
          this.abnormalLifeRatio = responseData.data.ratio;
        }
      });
    },
    drawAbnormalRatioChart() {
      setTimeout(() => {
        let abnormalRatioChart = this.$echarts.init(document.getElementById("abnormalRatioChart"));
        var option = {
          title: {
            show:true,
            text: '异常物料刀具寿命比例TOP 10',
            x: 'center',
            y: 'top'
          },
          grid:{
